...
function FAQ(props) {
    ...
    return (<div>
        ...
        {helpItem(classes, "9. How is assessment tool, checklist and area of concern linked to each other.", "Checklist can be created only after its assessment tool exists. Area of concerns can be created without assessment tool, but they are useful only when they are linked to some checklist. Area of concern is allowed to be created without any checklist because one area of concern can be used by multiple checklist.")}
        {helpItem(classes, "10. How are the scores calculated when the user hasn't filled certain checkpoints", "For calculation of scores only the filled checkpoints are considered in the denominator. The reason is that, we cannot assume any score for the unfilled checkpoints hence it is ignored in both numerator and denominator.")}
        <Paper className={classes.root} elevation={1}>
