...
export const GunakReferenceInput = ({label, source, optionText, record, onChange = () => {}, sort = "name", filter = {}, mandatory = true, perPage = 100, autoComplete = false}) => {
    return <ReferenceInput label={label} source={`${source}Id`} reference={source} validate={mandatory ? [required("Mandatory")] : []} perPage={perPage}
                           resource={source} sort={sort} filter={filter} style={{width: 500}} onChange={onChange}>
